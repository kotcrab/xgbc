...
fun generateOpCodes(emu: Emulator, cpu: Cpu, proc: OpCodesProcessor, op: Array<Instr?>) {
    ...
    op[0x07] = Instr(1, 4, "RLCA", { proc.rlcReg(Cpu.REG_A) })
    ...
    op[0x0F] = Instr(1, 4, "RRCA", { proc.rrcReg(Cpu.REG_A) })
    ...
    op[0x17] = Instr(1, 4, "RLA", { proc.rlReg(Cpu.REG_A) })
...
fun generateExtOpCodes(emu: Emulator, cpu: Cpu, proc: OpCodesProcessor, op: Array<Instr?>) {
    ...
    op[0x06] = Instr(2, 16, "RLC (HL)", {
        ...
        emu.write(addr, proc.rlc(emu.read(addr)))
    })
    ...
    op[0x0E] = Instr(2, 16, "RRC (HL)", {
        val addr = cpu.readReg16(Cpu.REG_HL)
        emu.write(addr, proc.rrc(emu.read(addr)))
    })
    ...
    op[0x16] = Instr(2, 16, "RL (HL)", {
        val addr = cpu.readReg16(Cpu.REG_HL)
        emu.write(addr, proc.rl(emu.read(addr)))
    })
    ...
    op[0x1E] = Instr(2, 16, "RR (HL)", {
        val addr = cpu.readReg16(Cpu.REG_HL)
        emu.write(addr, proc.rr(emu.read(addr)))
    })
